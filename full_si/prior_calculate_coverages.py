...
mdn_beta_bounds = []
true_beta_bounds = []
print("Calculating coverages drawn from prior.")
beta_draws = np.random.gamma(shape = prior_params[0], scale = 1/prior_params[1], size = coverage_iterations)
results = {"mdn_abc": {"draws": beta_draws, "means": [], "medians": [], "ranks": [], "percentiles": []}, "analytical": {"draws": beta_draws, "means": [], "medians": [], "ranks": [], "percentiles": []}}
for i in range(coverage_iterations):
	print("Iteration: " + str(i) + " out of " + str(coverage_iterations))
	samp = epidemic_utils.simulate_SI_gillespie(true_network, beta_draws[i], i_list, time_steps)
	...
	for j in range(num_training_samples):
		if euclidean_distances[j] <= percentile_value:
			accepted_thetas.append(training_betas[j])
	...
	for percentile in percentiles_of_interest:
		...
		if beta_draws[i] < mdn_beta_upper_bound and beta_draws[i] > mdn_beta_lower_bound:
		...
		if beta_draws[i] < true_beta_upper_bound and beta_draws[i] > true_beta_lower_bound:
	...
	mdn_beta_rank = sum(accepted_betas < beta_draws[i])
	results["mdn_abc"]["ranks"].append(mdn_beta_rank)
	true_beta_rank = sum(samp_true_draws < beta_draws[i])
	results["analytical"]["ranks"].append(true_beta_rank)
	mdn_beta_percentile = stats.percentileofscore(accepted_betas, beta_draws[i])
	results["mdn_abc"]["percentiles"].append(mdn_beta_percentile)
	true_beta_percentile = stats.percentileofscore(samp_true_draws, beta_draws[i])
...
mdn_abc_beta_coverage_file = open("abc/prior_coverages_mdn_abc.pkl","wb")
pickle.dump(mdn_beta_coverages, mdn_abc_beta_coverage_file)
mdn_abc_beta_coverage_file.close()
analytical_coverage_file = open("abc/prior_coverages_analytical.pkl","wb")
