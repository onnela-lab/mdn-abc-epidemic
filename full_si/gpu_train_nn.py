...
true_output_file = open("true_epidemic/true_nn_input.pkl", "rb")
true_output = pickle.load(true_output_file)
len_output = len(true_output) # When doing time inference, there's actually two elements to the output (the output and the times)	
	 
num_components = 2
batch_size = 500
num_features = 15 
num_param = 1
compressor_layers = [len_output, 80, 60, 30, 20, num_features]
mdn_layers = [num_features, 10, 10, num_components]
...
for key, path in paths.items():
    ...
    print(str(key) + "size: " + str(len(samples["output"]))) 
...
# The relevant neural nets are defined in vaccine_nn_utils.
# But since our dimension is now high, let's get ourselves 50 nodes in the two hidden layers for now.
compressor = vaccine_nn_utils.DenseStack(compressor_layers, th.nn.Tanh())
module = vaccine_nn_utils.MixtureDensityNetwork_multi_param_gamma(compressor, mdn_layers, num_param, th.nn.Tanh())
    
"""
Optimization and Training
"""
    
#M: Need to define an optimizer and a scheduler.
